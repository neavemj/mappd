...
rule associate_hostTaxid_genbank:
    ...
    shell:
        # cat's the results from all samples together
        # cuts the seventh field (taxid), figures out the taxid abundances
        # by counting up how many times each one occurs and sorting by most abundant
        # the first sed is required to remove the whitespace generated by uniq -c
        # then uses head to retrieve as many host taxids as required,
        # adds a tab before and after each taxid to ensure a
        # clean match
        """
        grep \
            -f <(cat {input} | cut -f 7 | sort | uniq -c | sort -nrk 1 | sed "s/^\s*//g" | \
            cut -f 2 -d " " | head -n {params.hosts_to_download} | sed "{params.sed_pat}") \
